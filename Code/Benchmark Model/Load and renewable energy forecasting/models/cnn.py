...
class CNNNet(nn.Module):
    def __init__(self,
                 ...
                 dropout, hidden_layers, kernel_size, stride, pooling, normalization
                 ):
        super().__init__()
        ...
        modules = list()
        last_channels = len(self.external_features) + len(self.history_column_names)
        padding, dilation = 0, 1
        L_in = self.sliding_window + 1
        ...
        self.final_layer = nn.Sequential(
            ...
            nn.LeakyReLU(),
            nn.Dropout(p=dropout),
            nn.Linear(1024, len(target_val_column_names)),
        )

    def forward(self, x):
        N, C_in, L_in = x.shape[0], len(self.history_column_names) + len(self.external_features), self.sliding_window+1
...
def grid_search_cnn(config, num_files):
    # set random seed
    ...
    for file in file_list:
        ...
        if not config['exp_params']['test_flag']:
            ...
            Pytorch_DNN_validation(os.path.join(data_folder, file), param_dict_list, cur_log_dir, config, CNN_exp)
            """
            ...
            param_dict = {
                ...
                'pooling': selected_params.split('_')[6],
        ...
        Pytorch_DNN_testing(os.path.join(data_folder, file), param_dict, cur_log_dir, config, CNN_exp)
...
if __name__ == '__main__':
    ...
    grid_search_cnn(config, num_files=args['num_files'])
