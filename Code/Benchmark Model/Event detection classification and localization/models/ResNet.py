...
class ResNetClassifier_(ResNetClassifier):
    def __init__(self,
                 nb_epochs,
                 batch_size,
                 callbacks,
...
class ResNet_exp(Keras_DNN_exp):
...
def grid_search_ResNet(config):
    ...
    if not config['exp_params']['test_flag']:
        Keras_DNN_validation(data_path, param_dict_list, log_dir, config, ResNet_exp)
    ...
    Keras_DNN_testing(data_path, param_dict, log_dir, config, ResNet_exp)
...
if __name__ == '__main__':
    ...
    grid_search_ResNet(config)

    """
    std:
    summary: {'#samples': 110, 'fault': 0.7451655761118677, 'location': 0.2729965156794425, 'starttime': 32.09146341463415}    
    """