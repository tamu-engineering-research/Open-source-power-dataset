...
class ClassificationDataset:
    def __init__(self, root, train_ratio=0.8):
        ...
        self.data_path = os.path.join(root, 'processed_dataset', 'classification.pkl')
    ...
    def load(self, batch_size, shuffle):
        ...
        train_dataset = TSTrainDataset(
            torch.from_numpy(self.data['feature_list'][train_idx]).to(torch.float),
            torch.from_numpy(self.data['label_list'][train_idx]).to(torch.long)
        )
        ...
        test_dataset = TSTestDataset(
            torch.from_numpy(self.data['feature_list'][test_idx]).to(torch.float)
        )
